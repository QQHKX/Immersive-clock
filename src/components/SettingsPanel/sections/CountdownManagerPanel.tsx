...
import { FormSection, FormRow, FormInput, FormButton, FormButtonGroup, FormSlider } from '../../FormComponents';
...
export const CountdownManagerPanel: React.FC<CountdownManagerPanelProps> = ({ onRegisterSave }) => {
  ...
  useEffect(() => {
    onRegisterSave?.(() => {
      ...
      const normalized: CountdownItem[] = items.map((it, idx) => ({
        id: it.id,
        kind: it.kind,
        name: (it.name && it.name.trim().length > 0) ? it.name.trim() : (it.kind === 'gaokao' ? '高考倒计时' : '自定义事件'),
        targetDate: it.kind === 'custom' ? (it.targetDate && it.targetDate.trim()) || '' : undefined,
        ...
        textOpacity: typeof it.textOpacity === 'number' ? it.textOpacity : 1,
        digitColor: it.digitColor && it.digitColor.trim().length > 0 ? it.digitColor.trim() : undefined,
        digitOpacity: typeof it.digitOpacity === 'number' ? it.digitOpacity : undefined,
        order: idx,
      }));
  ...
  const addCustom = useCallback(() => {
    ...
    setItems([...items, { id, kind: 'custom', name: '期末考试', targetDate: '', order: nextOrder, bgOpacity: 0, textOpacity: 1 }]);
  }, [items]);

  const addGaokao = useCallback(() => {
    const id = `gaokao-${Date.now()}`;
    const nextOrder = items.length;
    setItems([...items, { id, kind: 'gaokao', name: '高考倒计时', order: nextOrder, bgOpacity: 0, textOpacity: 1 }]);
  }, [items]);
  ...
  const onDragOver = useCallback((e: React.DragEvent<HTMLDivElement>, overId: string) => {
    e.preventDefault();
  ...
  return (
    <div className={styles.section}>
      {/* 删除标题 */}
      <p className={styles.helpText}>添加多个倒计时项目，并可拖动排序；自定义名称、目标日期与颜色。</p>
        ...
        {items.map((it) => (
          ...
          >
            <div className={styles.periodDisplay}>
              ...
              <FormButtonGroup>
                <FormButton variant="danger" onClick={() => removeItem(it.id)}>删除</FormButton>
              </FormButtonGroup>
            </div>
            ...
            <div className={styles.editFormCompact}>
              <FormRow gap="sm" align="center">
                ...
                />
                <FormSlider label="背景透明度" min={0} max={1} step={0.01} value={typeof it.bgOpacity === 'number' ? it.bgOpacity : 0} onChange={(v) => updateItem(it.id, { bgOpacity: v })} formatValue={(v) => `${Math.round(v * 100)}%`} />
                <FormInput
                  label="文字色"
                  type="color"
                  ...
                  style={{ width: 36, height: 36, padding: 0 }}
                />
                <FormSlider label="文字透明度" min={0} max={1} step={0.01} value={typeof it.textOpacity === 'number' ? it.textOpacity : 1} onChange={(v) => updateItem(it.id, { textOpacity: v })} formatValue={(v) => `${Math.round(v * 100)}%`} />
                <FormInput
                  label="数字颜色"
                  type="color"
                ...
                <FormSlider
                  ...
                  max={1}
                  step={0.01}
                  value={typeof it.digitOpacity === 'number' ? it.digitOpacity : (typeof study.digitOpacity === 'number' ? study.digitOpacity : 1)}
      ...
      <FormButtonGroup align="left">
        <FormButton variant="secondary" onClick={addGaokao}>添加高考倒计时</FormButton>
        <FormButton variant="primary" onClick={addCustom}>添加自定义倒计时</FormButton>
      </FormButtonGroup>
    </div>
  );
};
