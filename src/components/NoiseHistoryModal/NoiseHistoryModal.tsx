...
import { FormSection, FormButtonGroup, FormButton } from '../FormComponents';
...
const NoiseHistoryModal: React.FC<NoiseHistoryModalProps> = ({ isOpen, onClose }) => {
  ...
  const chart = useMemo(() => {
    if (!active) return { width: chartWidth, height: 160, padding: 36, path: '', xTicks: [] as number[], yTicks: [] as number[], thresholdY: 0, chartWidth: chartWidth };
            ...
            {reports.length === 0 ? (
              ...
              reports.map((r, idx) => (
                ...
                >
                  ...
                  <div className={styles.itemSub}>{new Date(r.start).toLocaleString()} - {new Date(r.end).toLocaleString()}</div>
                </div>
              ))
            )}
          </div>
            ...
            {active ? (
              <div className={styles.statsGrid}>
                ...
                <div className={styles.statItem}>
                  ...
                  <div className={styles.statValue}>{formatDuration(active.stats.noisyDurationMs)}</div>
                </div>
              </div>
            ) : (
          ...
          <FormSection title="历史图表">
            {active && active.series.length > 0 ? (
              <div ref={chartContainerRef} className={styles.chartRow}>
                <svg width={chart.width} height={active.chart.height} className={styles.chart} viewBox={`0 0 ${chart.width} ${active.chart.height}`}>
                  <defs>
                    <linearGradient id="historyAreaGradient" x1="0" y1="0" x2="0" y2={active.chart.height} gradientUnits="userSpaceOnUse">
                  ...
                  {/* 阈值线 */}
                  <line x1={chart.padding} y1={chart.thresholdY} x2={chart.width - chart.padding} y2={chart.thresholdY} stroke="#ff4757" strokeDasharray="4 4" strokeWidth={1} />
                  {/* 折线 */}
                  <path d={chart.path} stroke="#03DAC6" strokeWidth={2} fill="none" />
                  {/* 面积填充 */}
                  <path d={`${chart.path} L ${chart.width - chart.padding} ${active.chart.height - chart.padding} L ${chart.padding} ${active.chart.height - chart.padding} Z`} fill="url(#historyAreaGradient)" />
                </svg>
              </div>
            ) : (
