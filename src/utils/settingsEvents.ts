 ...
 */
export function broadcastSettingsEvent(eventName: SettingsEventName, detail?: SettingsEventDetail): void {
  try {
    const ev = new CustomEvent(eventName, { detail });
    window.dispatchEvent(ev);
  } catch (error) {
    console.error('广播设置事件失败:', eventName, error);
  }
}
 ...
 * @returns 取消订阅函数
 */
export function subscribeSettingsEvent(eventName: SettingsEventName, handler: (evt: CustomEvent) => void): () => void {
